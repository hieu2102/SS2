...
def getLargestValue(dict):
    ...
    return max
cat = ['foods', 'politics', 'sports','tech']
for c in cat:  
    print(c)
    text_data = []
    with open('C:\\\\Users\\USER\\Desktop\\SS2\\NLP\\training_data\\'+c+'.csv',encoding="utf8") as f:
        for line in f:
            tokens = prepare_text_for_lda(line)
            if random.random() > .99:
                try: 
    ...
    corpus = [dictionary.doc2bow(text) for text in text_data]
    pickle.dump(corpus, open('corpus.pkl', 'wb'))
    dictionary.save(c+'dictionary.gensim')
    ldamodel = gensim.models.ldamodel.LdaModel(corpus, num_topics = NUM_TOPICS, id2word=dictionary, passes=2805)
    ldamodel.save(c+'model5.gensim')
    # topics = ldamodel.print_topics(num_words=10)
    # # for topic in topics:
    #     # print(topic)
    # new_doc = "Watch The Top 9 YouTube Videos Of The Week,\"If you're looking to see the most popular YouTube videos of the week, look no further. Once again, we\'re bringing you the"
    # new_doc = prepare_text_for_lda(new_doc)
    # new_doc_bow = dictionary.doc2bow(new_doc)
    # print(new_doc)
    # print(new_doc_bow)
